...
public class Level {
	...
	public void changeTileAt(int x, int y) {
		...
		if (getTile(tiles[x + y * width]).isSolid()
			...
			if (!getTile(tiles[x + y * width]).isDamaged()) {
				tiles[x + y * width] = getTile(tiles[x + y * width])
						.getDamageTile().getID();
				getTile(tiles[x + y * width]).damage();
			} else {
				AnimatedTile anTile = (AnimatedTile) getTile(tiles[x + y
						* width]);
				anTile.nextFrame();
	...
	// UNFINISHED
	public boolean isTileCollision(int x, int y, int size, int xOffset,
			int yOffset) {
