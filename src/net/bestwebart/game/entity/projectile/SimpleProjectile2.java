...
public class SimpleProjectile2 extends Projectile {
	...
	while (nx != 0 && ny != 0) {
	    if (nx > 1) {
		if (!Game.level.isTileCollision((int) x + getSignedValue(nx), (int) y, size, xOffset, yOffset)) x += getSignedValue(nx);
		nx -= getSignedValue(nx);
	    } else {
		if (!Game.level.isTileCollision((int) (x + nx), (int) y, size, xOffset, yOffset)) x += nx;
		nx = 0;
	    }
	    if (ny > 1) {
		if (!Game.level.isTileCollision((int) x, (int) y + getSignedValue(ny), size, xOffset, yOffset)) y += getSignedValue(ny);
		ny -= getSignedValue(ny);
	    } else {
		if (!Game.level.isTileCollision((int)x, (int) (y + ny), size, xOffset, yOffset)) y += ny;
		ny = 0;
	    }
	}
    }
