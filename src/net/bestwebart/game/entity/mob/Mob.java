...
public abstract class Mob extends Entity {
	...
	while (nx != 0) {
	    if (nx > 1) {
		if (!isCollision(x + getSignedValue(nx), y)) x += getSignedValue(nx);
		nx -= getSignedValue(nx);
	    } else {
		if (!isCollision(x + nx, y)) x += nx;
		nx = 0;
	    }
	}
	...
	while (ny != 0) {
	    if (ny > 1) {
		if (!isCollision(x, y + getSignedValue(ny))) y += getSignedValue(ny);
		ny -= getSignedValue(ny);
	    } else {
		if (!isCollision(x, y + ny)) y += ny;
		ny = 0;
	    }
	}
    }
