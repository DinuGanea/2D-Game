...
public class Tonny extends Mob {
	...
	if (time % 60 == 0) {
	    ...
	    if (isCollision(x + nx, y)) nx *= -speed;
	    if (isCollision(x, ny + y)) ny *= -speed;
	    
	    if (ny > 0) animTile = (AnimatedTile) Tile.TONNY_DOWN;
	    else if (ny < 0) animTile = (AnimatedTile) Tile.TONNY_UP;
